...
```sh
npm run api    # :8787
npm run dev    # :5173 → proxies API via VITE_API_BASE default
```
...
| GET | `/api/targets` | Index page | Index and stock target list |
...
User opens /indices
  ...
  → GET /api/live-premium/NASDAQ_100         (live quotes + IOPV alignment)
  → POST /api/sync-limits/NASDAQ_100         (periodic, optional refresh)
```
