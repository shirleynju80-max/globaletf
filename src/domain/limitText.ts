...
export function parseDirectLimitFromAnnouncement(
  ...
  const classPatterns: Array<{ pattern: RegExp; confidence: number }> = [
    { pattern: new RegExp(`${shareClass}\\s*类基金份额限额(\\d+(?:\\.\\d+)?)万元`), confidence: 0.95 },
    { pattern: new RegExp(`${shareClass}\\s*类基金份额限额(\\d+(?:\\.\\d+)?)元`), confidence: 0.95 },
    { pattern: new RegExp(`${shareClass}类基金份额限额(\\d+(?:\\.\\d+)?)万元`), confidence: 0.95 },
    { pattern: new RegExp(`${shareClass}类基金份额限额(\\d+(?:\\.\\d+)?)元`), confidence: 0.95 },
    { pattern: new RegExp(`（QDII）${shareClass}[^。]{0,60}?不得超过(\\d+(?:\\.\\d+)?)元`), confidence: 0.9 },
    { pattern: new RegExp(`${shareClass}\\s*类[^。]{0,40}?限额(\\d+(?:\\.\\d+)?)万元`), confidence: 0.9 },
    { pattern: new RegExp(`下属${shareClass}类[^。]{0,60}?限额(\\d+(?:\\.\\d+)?)万元`), confidence: 0.9 }
  ];

  for (const { pattern, confidence } of classPatterns) {
    const match = normalized.match(pattern);
    if (match) {
      const limitAmountYuan = parseWanYuan(match[1]);
      return {
        status: "limited",
        limitAmountYuan,
        limitUnit: "per_day",
  ...
  if (fundCode) {
    ...
    const amounts = segment ? [...segment.matchAll(/(\d+(?:\.\d+)?)元/g)].map((match) => Number(match[1])) : [];
    const limitAmountYuan = amounts.filter((value) => Number.isFinite(value)).at(-1);
    if (limitAmountYuan != null && limitAmountYuan > 0) {
      return {
        status: "limited",
        limitAmountYuan,
        limitUnit: "per_day",
        confidence: 0.9,
        excerpt: `${fundCode}…${limitAmountYuan}元`
      };
    }
  }
  ...
  const amountPatterns = [
    /限制申购金额[^0-9]{0,30}(\d+(?:\.\d+)?)/,
    /限制定期定额投资金额[^0-9]{0,30}(\d+(?:\.\d+)?)/
  ];
  for (const pattern of amountPatterns) {
    const match = normalized.match(pattern);
    if (match) {
      const limitAmountYuan = Number(match[1]);
      if (Number.isFinite(limitAmountYuan) && limitAmountYuan > 0) {
        return {
          status: "limited",
          limitAmountYuan,
          limitUnit: "per_day",
  ...
  const directPatterns = [
    /本公司直销渠道[^。]{0,80}?限额(\d+(?:\.\d+)?)万元/,
    /直销渠道个人投资者[^。]{0,80}?限额(\d+(?:\.\d+)?)万元/,
    /通过本公司直销渠道[^。]{0,80}?限额(\d+(?:\.\d+)?)万元/,
    /直销渠道[^。]{0,40}?限额(\d+(?:\.\d+)?)万元/
  ];
  for (const pattern of directPatterns) {
    ...
    if (match) {
      return {
        status: "limited",
        limitAmountYuan: parseWanYuan(match[1]),
        limitUnit: "per_day",
        confidence: 0.8,
        excerpt: match[0]
      };
...
function parseWanYuan(valueText: string): number {
  const value = Number(valueText);
  if (!Number.isFinite(value)) return 0;
  if (valueText.includes(".") || value < 1000) return value * 10000;
  return value;
}
